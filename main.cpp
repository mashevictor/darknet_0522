    ...
    else
    {
        ...
        if (!retrievedImageFlags[0] && !retrievedImageFlags[1] && !retrievedImageFlags[2])
            return 0;
                ...
                char path[60];
                strftime(path, 60, "%Y_%m_%d", timeinfo);
                char strPath[100];
                sprintf(strPath, "%s.avi", path);//将创建文件的命令存入cmdchar中
                VideoWriter writer;
                writer.open(strPath, CV_FOURCC('X', 'V', 'I', 'D'), 25, Size(640, 480), true);
    for(;;)
    {
        Mat depthMap;
        Mat validDepthMap;
        ...
        else
        {
            ...
            if(retrievedImageFlags[3] && capture.retrieve( myImage, CAP_OPENNI_BGR_IMAGE ))
            {
                char* cstr = new char[120];
                sprintf(cstr, "%s%d%s", "/home/victor/Videos/videojpg/abcd", n++, ".jpg");
                imshow( "my image", myImage);
                imwrite(cstr, myImage);
		writer<<myImage;
            }
